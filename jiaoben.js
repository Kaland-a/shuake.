...
答案：`;
    ...
    return new Promise((resolve) => {
      ...
      _GM_xmlhttpRequest({
        ...
        headers: {
          ...
          "u": (_self.uid || (typeof _self.getCookie === 'function' ? (_self.getCookie("UID") || _self.getCookie("_uid")) : "") || ""),
        ...
        onload: (response) => {
          try {
            ...
            if (isOllama) {
              ...
              if (question.type === "0") {
                // 单选题 - 提取字母并转换为选项文本
                console.log(`[AI答题] 单选题原始答案: "${aiAnswer}"`);
                console.log(`[AI答题] 选项数量: ${question.optionsText ? question.optionsText.length : 0}`);
                
                // 方法1: 查找独立的字母（A、B、C等）
                let match = aiAnswer.match(/\b([A-Z])\b/);
                if (!match) {
                  // 方法2: 查找任何大写字母
                  match = aiAnswer.match(/([A-Z])/);
                }
                
                if (match) {
                  const letter = match[1] || match[0];
                  const idx = letter.charCodeAt(0) - 65;  // A=0, B=1, C=2...
                  console.log(`[AI答题] 找到字母: ${letter}, 索引: ${idx}`);
                  
                  if (question.optionsText && idx >= 0 && idx < question.optionsText.length) {
                    answers = [question.optionsText[idx]];
                    console.log(`[AI答题] 单选题解析成功: ${letter} -> ${answers[0]}`);
                  } else {
                    console.log(`[AI答题] 索引 ${idx} 超出选项范围 (0-${question.optionsText ? question.optionsText.length - 1 : -1})`);
                  }
                } else {
                  console.log(`[AI答题] 未找到选项字母`);
                }
              } else if (question.type === "1") {
                // 多选题 - 提取所有字母并转换为选项文本
                console.log(`[AI答题] 多选题原始答案: "${aiAnswer}"`);
                console.log(`[AI答题] 选项数量: ${question.optionsText ? question.optionsText.length : 0}`);
                
                // 方法1: 查找所有独立的字母
                let letters = aiAnswer.match(/\b([A-Z])\b/g) || [];
                if (letters.length === 0) {
                  // 方法2: 查找任何大写字母
                  letters = aiAnswer.match(/([A-Z])/g) || [];
                }
                
                console.log(`[AI答题] 找到字母: ${letters.join(',')}`);
                
                answers = [];
                letters.forEach(letter => {
                  const cleanLetter = letter.replace(/\b/g, ''); // 清理可能的单词边界
                  const idx = cleanLetter.charCodeAt(0) - 65;
                  
                  if (question.optionsText && idx >= 0 && idx < question.optionsText.length) {
                    answers.push(question.optionsText[idx]);
                  } else {
                    console.log(`[AI答题] 字母 ${cleanLetter} (索引${idx}) 超出选项范围`);
                  }
                });
                
                if (answers.length > 0) {
                  console.log(`[AI答题] 多选题解析成功: ${letters.join(',')} -> ${answers.length}个选项`);
                } else {
                  console.log(`[AI答题] 多选题未找到有效答案`);
                }
              } else if (question.type === "3") {
                // 判断题
                console.log(`[AI答题] 判断题原始答案: "${aiAnswer}"`);
                const lower = aiAnswer.toLowerCase();
                if (aiAnswer.includes("正确") || lower.includes("true") || lower.includes("yes") || aiAnswer.includes("对") || aiAnswer.includes("✓")) {
                  answers = ["正确"];
                  console.log("[AI答题] 判断题解析结果: 正确");
                } else if (aiAnswer.includes("错误") || lower.includes("false") || lower.includes("no") || aiAnswer.includes("错") || aiAnswer.includes("×")) {
                  answers = ["错误"];
                  console.log("[AI答题] 判断题解析结果: 错误");
                } else {
                  console.log(`[AI答题] 判断题无法识别: ${aiAnswer}`);
                }
              } else {
                // 填空题、简答题等
                console.log(`[AI答题] 主观题原始答案: "${aiAnswer}"`);
                ...
                // 如果答案以选项字母开头，去掉它
                cleanedAnswer = cleanedAnswer.replace(/^[A-Z]\.\s*/, '');
                
                answers = [cleanedAnswer];
                console.log(`[AI答题] 主观题解析结果: ${cleanedAnswer.substring(0, 50)}${cleanedAnswer.length > 50 ? '...' : ''}`);
              }

              if (answers.length > 0) {
                console.log(`[AI答题] 解析成功，返回 ${answers.length} 个答案: ${JSON.stringify(answers)}`);
                resolve({
                  code: 200,
                  data: {
                    answer: answers,
                ...
                console.log(`[AI答题] 题型: ${question.type}, 原始答案: "${aiAnswer}"`);
                console.log(`[AI答题] 可用选项: ${question.optionsText ? JSON.stringify(question.optionsText) : '无'}`);
              ...
              if (apiResponse.code === 200 && apiResponse.data && apiResponse.data.answer) {
                console.log("[AI答题] Python服务原始返回:", JSON.stringify(apiResponse.data.answer));
                
                // 如果返回的是单个字母且长度为1，可能是需要转换为选项文本
                if (apiResponse.data.answer.length === 1 && 
                    apiResponse.data.answer[0].length === 1 && 
                    /^[A-Z]$/.test(apiResponse.data.answer[0]) &&
                    question.optionsText && question.optionsText.length > 0) {
                  
                  const letter = apiResponse.data.answer[0];
                  const idx = letter.charCodeAt(0) - 65; // A=0, B=1, C=2...
                  
                  console.log(`[AI答题] 检测到字母答案 ${letter}，尝试转换为选项文本`);
                  
                  if (idx >= 0 && idx < question.optionsText.length) {
                    processedAnswers = [question.optionsText[idx]];
                    console.log(`[AI答题] 字母 ${letter} 转换为选项: ${processedAnswers[0]}`);
                  } else {
                    console.log(`[AI答题] 字母 ${letter} 索引 ${idx} 超出范围，保持原答案`);
                    processedAnswers = apiResponse.data.answer;
                  }
                } else {
                  // 检查是否是多个字母（多选题）
                  if (apiResponse.data.answer.length > 1 && 
                           apiResponse.data.answer.every(ans => ans.length === 1 && /^[A-Z]$/.test(ans)) &&
                           question.optionsText && question.optionsText.length > 0) {
                    
                    console.log(`[AI答题] 检测到多选题字母答案: ${apiResponse.data.answer.join(',')}`);
                    
                    processedAnswers = [];
                    apiResponse.data.answer.forEach(letter => {
                      const idx = letter.charCodeAt(0) - 65;
                      if (idx >= 0 && idx < question.optionsText.length) {
                        processedAnswers.push(question.optionsText[idx]);
                      }
                    });
                    
                    console.log(`[AI答题] 多选题转换结果:`, processedAnswers);
                  } else if (question.type === "1" && apiResponse.data.answer.length === 1 && 
                            /^[A-Z,]+$/.test(apiResponse.data.answer[0]) &&
                            question.optionsText && question.optionsText.length > 0) {
                    // 多选题处理：检查是否是字母组合
                    const letters = apiResponse.data.answer[0].split(',');
                    processedAnswers = [];
                    
                    console.log(`[AI答题] 检测到多选题字母答案: ${letters.join(',')}`);
                    ...
                    letters.forEach(letter => {
                      ...
                      if (idx >= 0 && idx < question.optionsText.length) {
                        ...
                        console.log(`[AI答题] 多选题字母 ${letter} 索引 ${idx} 超出范围`);
                      }
                    });
                    
                    if (processedAnswers.length > 0) {
                      console.log(`[AI答题] 多选题字母转换为选项: ${JSON.stringify(processedAnswers)}`);
                    } else {
                      console.log(`[AI答题] 多选题字母转换失败，保持原答案`);
                      processedAnswers = apiResponse.data.answer;
                    }
                  } else {
                    // 其他情况，保持原答案
      ...
      __publicField(this, "fillQuestion", (question) => {
        ...
        if (question.type === "0" || question.type === "1") {
          ...
          question.answer.forEach((answer) => {
            ...
            // 检查是否是字母答案（A、B、C等）
            const isLetterAnswer = /^[A-Z]$/.test(cleanAnswer) || (question.type === "1" && /^[A-Z,]+$/.test(cleanAnswer));
            
            if (isLetterAnswer && question.optionsText && question.optionsText.length > 0) {
              // 如果是字母答案且有选项文本，将字母转换为选项文本
              console.log(`[答题] 检测到字母答案: ${cleanAnswer}，尝试转换为选项文本`);
              
              if (question.type === "0") {
                // 单选题
                const idx = cleanAnswer.charCodeAt(0) - 65; // A=0, B=1, C=2...
                if (idx >= 0 && idx < question.optionsText.length) {
                  const optionText = question.optionsText[idx];
                  console.log(`[答题] 字母 ${cleanAnswer} 转换为选项文本: ${optionText}`);
                  
                  // 查找匹配的选项
                  for (const key in question.options) {
                    if (key === optionText && !selectedKeys.has(key)) {
                      matched = true;
                ...
                letters.forEach(letter => {
                  ...
                  if (idx >= 0 && idx < question.optionsText.length) {
                    const optionText = question.optionsText[idx];
                    console.log(`[答题] 多选题字母 ${letter} 转换为选项文本: ${optionText}`);
  ...
  const useCxWorkLogic = async () => {
    ...
    const correctRate = await new CxQuestionHandler("zy").init();
    logStore.addLog(`答题完成，正确率: ${correctRate}%`, "primary");
  };
  const useCxExamLogic = async () => {
    ...
    logStore.addLog(`正在解析题目, 请等待5s`, "warning");
    const correctRate = await new CxQuestionHandler("ks").init();
    logStore.addLog(`答题完成，正确率: ${correctRate}%`, "primary");
  ...
  class DgutQuestionHandler extends BaseQuestionHandler {
    constructor() {
      super();
    }
      ...
      if (this.questions.length) {
        ...
        for (const [index, question] of this.questions.entries()) {
          ...
          if (answerData.code === 200) {
            question.answer = answerData.data.answer;
            this.fillQuestion(question);
            this.addLog(`第${index + 1}题搜索成功`, "success");
            this.addLog(`剩余次数:${answerData.data.num}`, "primary");
            this.correctNum += 1;
          } else {
            this.addLog(`第${index + 1}题搜索失败，点击上方答题查看原因`, "danger");
            question.answer[0] = answerData.msg;
          }
    ...
    // 题目在 li.question-item 中
    const questionElements = this._document.querySelectorAll('li.question-item');
    this.addQuestions(questionElements);
};
    ...
    questionElements.forEach((questionElement) => {
        // 获取题号和题型
        const titleDiv = questionElement.querySelector('.title .gray');
        const titleText = titleDiv?.textContent || "";
        
        // 提取题型
        let questionTypeText = "单选题";
        if (titleText.includes('判断')) {
            questionTypeText = "判断题";
        } else if (titleText.includes('多选')) {
            questionTypeText = "多选题";
        } else if (titleText.includes('单选')) {
            questionTypeText = "单选题";
        }
        
        // 获取题目内容
        const questionTitleElement = questionElement.querySelector('.question-title');
        const questionTitle = this.removeHtml(questionTitleElement?.textContent || "");
        
        // ========== 关键修改：选项获取逻辑 ==========
        const answerArea = questionElement.querySelector('.answer-area');
        const optionsObject = {};
        const optionTexts = [];
        
        if (questionTypeText === "判断题") {
            // 判断题：查找带 value="true/false" 的 label
            const optionLabels = answerArea?.querySelectorAll('label[role="radio"]');
            optionLabels?.forEach((label) => {
                const input = label.querySelector('input[type="radio"]');
                const value = input?.getAttribute('value');
                
                if (value === 'true') {
                    optionsObject['正确'] = label;
                    optionTexts.push('正确');
                } else if (value === 'false') {
                    optionsObject['错误'] = label;
                    optionTexts.push('错误');
                }
            });
        } else {
            // 单选/多选题：查找所有选项，从完整文本中提取
            // 选项格式通常是：A. 选项内容
            const optionLabels = answerArea?.querySelectorAll('label');
            ...
            optionLabels?.forEach((label) => {
                ...
                // 移除选项字母前缀（A. B. C. D.）
                const cleanText = fullText.replace(/^[A-Z]\.\s*/, '').trim();
                
                if (cleanText) {
                    optionsObject[cleanText] = label;
        ...
        this.questions.push({
            element: questionElement,
            type: this.questionType[questionTypeText] || "0",
            title: this.clean(questionTitle),
            optionsText: optionTexts,
            options: optionsObject,
            answer: [],
    ...
    if (question.type === "3") {
        ...
        if (answerText.match(/(是|对|正确|√|✓|true|True|T|yes|1)/)) {
            targetAnswer = "正确";
        }
        
        const optionLabel = question.options[targetAnswer];
        if (optionLabel) {
            // 直接点击label
            optionLabel.click();
            console.log(`判断题已选择: ${targetAnswer}`);
        }
        
    } else if (question.type === "0" || question.type === "1") {
        // ========== 单选/多选题逻辑 ==========
        const selectedKeys = new Set();
        
        question.answer.forEach((answer) => {
            const cleanAnswer = this.removeHtml(answer).trim();
            let matched = false;
            ...
            for (const key in question.options) {
                if (key === cleanAnswer && !selectedKeys.has(key)) {
                    ...
                    // 检查是否已选中
                    const isChecked = label.querySelector('.ul-radio__input.is-checked, .ul-checkbox__input.is-checked');
                    
                    if (!isChecked) {
                        label.click();
                        console.log(`已选择: ${key.substring(0, 30)}...`);
                    }
                    break;
                }
            }
            ...
            if (!matched && useSimilarity) {
                ...
                if (bestMatch && !selectedKeys.has(bestMatch.key)) {
                    ...
                    const label = question.options[bestMatch.key];
                    const isChecked = label.querySelector('.ul-radio__input.is-checked, .ul-checkbox__input.is-checked');
                    
                    if (!isChecked) {
                        label.click();
                        console.log(`相似度匹配已选择: ${bestMatch.key.substring(0, 30)}... (${bestMatch.similarity}%)`);
                    }
                }
            }
        });
  ...
  const useDgutAnswerLogic = async () => {
    const logStore = useLogStore();
    const configStore = useConfigStore();
    
    logStore.addLog(`进入东莞理工优学院答题页面`, "primary");
    logStore.addLog(`正在解析题目, 请等待5s`, "warning");
    
    // 等待页面完全加载
    await sleep(3);
    
    const handler = new DgutQuestionHandler();
    const correctRate = await handler.init();
    
    logStore.addLog(`答题完成，正确率: ${correctRate.toFixed(2)}%`, "success");
    
    if (configStore.platformParams.dgut.parts[0].params[0].value) {
        logStore.addLog("自动切换已开启", "success");
        await sleep(2);
        
        // 查找提交按钮
        const submitBtn = document.querySelector('button.submit-btn, button[class*="submit"]');
        if (submitBtn && !submitBtn.disabled) {
            logStore.addLog("尝试自动提交", "primary");
            submitBtn.click();
        } else {
            logStore.addLog("未找到提交按钮或已禁用", "warning");
        }
    } else {
        logStore.addLog("已经关闭自动切换，在设置里可更改", "danger");
    }
  ...
  const _sfc_main$3 = /* @__PURE__ */ vue.defineComponent({
    ...
    setup(__props, { emit: __emit }) {
      ...
      const urlLogicPairs = [
        ...
        { 
          keyword: "lms.dgut.edu.cn/homework/quiz", 
          logic: useDgutAnswerLogic 
        }
      ];
  ...
  const _sfc_main = /* @__PURE__ */ vue.defineComponent({
    ...
    setup(__props) {
      ...
      else if (url2.includes("lms.dgut.edu.cn"))
        configStore.platformName = "dgut";
      return (_ctx, _cache) => {
